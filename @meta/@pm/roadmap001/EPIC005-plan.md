...
- [ ] Phase 5.1: Tie each resource to one Elixir owner; prove owner-death cleanup and idempotent close.
- [ ] Phase 5.2: Run many encoders/decoders concurrently across processes; assert isolation and baseline-returning counters.
- [ ] Phase 5.3: Prove large-frame encode/decode leaves normal schedulers responsive (timed heartbeat).
- [ ] Phase 5.4: Cover error paths: invalid rates/channels, empty PCM, oversized frames, truncated/corrupt packets, non-binary.
- [ ] Phase 5.5: Fuzz random/corrupt input in a disposable child BEAM; assert clean survivability and panic containment.
- [ ] Phase 5.6: Add resource-baseline and encode/decode throughput smoke checks.
- [ ] Phase 5.7: Pass the epic gate, verify every Epic 5 criterion, and prepare the focused commit.
...
- [ ] No leaked resource or growing counter after concurrent/repeated lifecycle.
- [ ] Large-frame codec work never blocks a normal scheduler.
- [ ] All hostile/invalid inputs map to stable tagged errors; child BEAM survives fuzz.
- [ ] Close and owner death are deterministic and idempotent.
